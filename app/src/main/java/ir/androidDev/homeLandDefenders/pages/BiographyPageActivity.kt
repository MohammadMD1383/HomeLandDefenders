...
class BiographyPageActivity : CustomizableActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		if (!autoDownload) binding.ivBiographyPageActivityTopPhoto.setOnClickListener {
			if (!imageLoaded) {
				Picasso.get().load(imageUrl).centerInside().fit().placeholder(placeholder)
					.into(binding.ivBiographyPageActivityTopPhoto, object : Callback {
						...
						override fun onError(e: Exception?) {
							binding.ivBiographyPageActivityTopPhoto.setImageDrawable(dlPlaceholder)
						}
					})
			}
		}
	...
	private fun getAndShowData() {
		...
		if (imageUrl.contains("void(0)", true)) binding.frBiographyPageActivityImageContainer.visibility = View.GONE
		...
		Picasso.get().load(imageUrl).centerInside().fit().networkPolicy(NetworkPolicy.OFFLINE).placeholder(placeholder)
			.into(binding.ivBiographyPageActivityTopPhoto, object : Callback {
				...
				override fun onError(e: Exception?) {
					...
					else binding.ivBiographyPageActivityTopPhoto.setImageDrawable(dlPlaceholder)
				}
			})
